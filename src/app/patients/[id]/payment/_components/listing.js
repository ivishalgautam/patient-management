...
export default function Listing({ patientId }) {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: [`patient-payments-${patientId}`, searchParamStr],
    queryFn: () => fetchPaymentsByPatient(patientId, searchParamStr),
    enabled: !!patientId,
  });
  ...
  const updateMutation = useMutation({
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries([`patient-payments-${patientId}`]);
    },
  });
